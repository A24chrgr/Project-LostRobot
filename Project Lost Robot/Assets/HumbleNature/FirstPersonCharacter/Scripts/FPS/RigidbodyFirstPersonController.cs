...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        public class MovementSettings
        {
            ...
            {
                return Input.GetKey(KeyCode.LeftShift) || inputRun;
            }
        ...
        public AudioSource myAudioJump;
        public AudioClip sFootsteps;
        ...
        private void Update()
        {
            ...
            if ((JumpAxis || Input.GetButton("Jump")) && !m_Jump)
            {
                m_Jump = true;
            }
        }
        ...
        private void FixedUpdate()
        {
            ...
            if (m_IsGrounded)
            {
                ...
                if (m_Jump)
                {
                    if (myAudio)
                    {
                        myAudioJump.clip = sJump;
                        myAudioJump.volume = UnityEngine.Random.Range(0.1f, 1f);
                        myAudioJump.pitch = UnityEngine.Random.Range(0.8f, 1f);
                        if (!myAudioJump.isPlaying)
                        {
                            myAudioJump.Play();
                        }
                    }
                    ...
                    //m_RigidBody.AddForce((desiredMove * SlopeMultiplier()) + new Vector3(0f, movementSettings.JumpForce, 0f), ForceMode.Impulse);
        ...
        {
            RunAxis += new Vector2(Input.GetAxis("Horizontal"),Input.GetAxis("Vertical"));
